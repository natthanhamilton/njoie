...
slate.rte = {
	...
	wrapIframe: function (options) {
		options.$iframes.each(function () {
			...
			$(this).wrap(
				'<div class="' + options.iframeWrapperClass + '"></div>'
			);
...
theme.Header = (function () {
	...
	function positionFullWidthDropdowns() {
		...
		$listWithCenteredDropdown.each(function () {
			...
			var fullWidthDropdownOffset =
				$hasCenteredDropdown.position().top + 41;
...
theme.MobileNav = (function () {
	...
	function goToSubnav(target) {
		...
		var $menuTitle = $targetMenu.find(
			'[data-menu-title=' + menuLevel + ']'
		);
...
theme.Slideshow = (function () {
	...
	function slideshow(el, sectionId) {
		...
		if (this.adaptHeight) {
			...
			$(window).resize(
				$.debounce(50, this.setSlideshowHeight.bind(this))
			);
		}
...
theme.Video = (function () {
	...
	function initEvents() {
		$(document).on('click.videoPlayer', selectors.playVideoBtn, function (
			evt
		) {
			...
			$.debounce(200, function () {
				...
				if (isMobile()) {
					for (key in videos) {
						if (videos.hasOwnProperty(key)) {
							if (
								videos[key].$videoWrapper.hasClass(
									classes.playing
								)
							) {
								if (!fullscreen) {
			...
			document.querySelector(selectors.predictiveSearchWrapper),
			{
				allowTouchMove: function (element) {
					return (
						...
						$(element).parents(selectors.predictiveSearchWrapper)
							.length === 1
					);
				},
			}
...
theme.Cart = (function () {
	...
	Cart.prototype = _.assignIn({}, Cart.prototype, {
		...
		_updateLiveRegion: function (item) {
			...
			$liveRegion
				.html(this._liveRegionText(item))
		...
		_createCartDiscountList: function (cart) {
			return $.map(
				...
				function (discount) {
					...
					$discount
						.find(selectors.cartDiscountTitle)
						.text(discount.title);
					$discount
		...
		_createUnitPrice: function (
			...
			$(selectors.unitPriceBaseUnit, $unitPriceGroup).text(
				unitPriceBaseUnit
			);
		...
		_hideQuantityErrorMessage: function () {
			...
			$(selectors.inputQty, this.$container).removeClass(
				classes.inputError
			);
		},
